...
1. Read `fontOptions` and `defaults.fonts` from `references/themes.json`.
2. Present both font roles and their available options in two short lists:
   - UI font: recommend `defaults.fonts.ui`, then list the remaining UI options.
   - Code font: recommend `defaults.fonts.code`, then list the remaining code options.
